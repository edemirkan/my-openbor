...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale1x1_n16, &scale1x2_n16, &scale1x3_n16, &scale1x4_n16};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale1x1_n32, &scale1x2_n32, &scale1x3_n32, &scale1x4_n32};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale2x1_n16, &scale2x2_n16, &scale2x3_n16, &scale2x4_n16};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale2x1_n32, &scale2x2_n32, &scale2x3_n32, &scale2x4_n32};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
void scale3x1_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x1_c16(src, dst, sw, sh, sp, dp);
		return;
	}
...
void scale3x2_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x2_c16(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
void scale3x3_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x3_c16(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
void scale3x4_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x4_c16(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale3x1_n16, &scale3x2_n16, &scale3x3_n16, &scale3x4_n16};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
void scale3x1_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x1_c32(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
void scale3x2_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x2_c32(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
void scale3x3_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x3_c32(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
void scale3x4_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale3x4_c32(src, dst, sw, sh, sp, dp);
		return;
	}
	uint32_t swl32 = swl & ~31;
	uint32_t sadd = sp - swl;
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale3x1_n32, &scale3x2_n32, &scale3x3_n32, &scale3x4_n32};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale4x1_n16, &scale4x2_n16, &scale4x3_n16, &scale4x4_n16};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
{
	const void (*func[4])(void *__restrict, void *__restrict, uint32_t, uint32_t, uint32_t, uint32_t) = {&scale4x1_n32, &scale4x2_n32, &scale4x3_n32, &scale4x4_n32};
	if (--ymul < 4)
		func[ymul](src, dst, sw, sh, sp, dp);
	return;
}
...
void scale5x_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp, uint32_t ymul)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale5x_c16(src, dst, sw, sh, sp, dp, ymul);
		return;
	}
...
void scale5x_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp, uint32_t ymul)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale5x_c32(src, dst, sw, sh, sp, dp, ymul);
		return;
	}
	void *__restrict dstsrc;
	for (; sh > 0; sh--, src = (uint8_t *)src + sp)
...
void scale6x_n16(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp, uint32_t ymul)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale6x_c16(src, dst, sw, sh, sp, dp, ymul);
		return;
	}
	void *__restrict dstsrc;
	for (; sh > 0; sh--, src = (uint8_t *)src + sp)
...
void scale6x_n32(void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp, uint32_t ymul)
{
	...
	if (((uintptr_t)src & 3) || ((uintptr_t)dst & 3) || (sp & 3) || (dp & 3))
	{
		scale6x_c32(src, dst, sw, sh, sp, dp, ymul);
		return;
	}
	void *__restrict dstsrc;
	for (; sh > 0; sh--, src = (uint8_t *)src + sp)
...
void scaler_n16(uint32_t xmul, uint32_t ymul, void *__restrict src, void *__restrict dst, uint32_t sw, uint32_t sh, uint32_t sp, uint32_t dp)
{
	...
	if ((--xmul < 6) && (--ymul < 6))
		func[xmul][ymul](src, dst, sw, sh, sp, dp);
	return;
}
