...
s_videomodes setupPreBlitProcessing(s_videomodes videomodes)
{
	...
	if (savedata.swfilter && (savedata.hwscale >= 2.0 || savedata.fullscreen))
	{
		...
		// edemirkan (2023-02-26) temp surface alignment start ->
		// if (screen) SDL_FreeSurface(screen);
		if (screen)
			free(screen->pixels);
		screen = NULL;
		int pitch = videomodes.hRes * 2 * (16 >> 3);
		uint32_t size = pitch * videomodes.vRes * 2;
		void *address = valloc(size);
		screen = SDL_CreateRGBSurfaceFrom(address, videomodes.hRes * 2, videomodes.vRes * 2, 16, pitch, masks[1][0], masks[1][1], masks[1][2], masks[1][3]); // edemirkan (2023-02-26) temp surface allignment.
		// screen = SDL_CreateRGBSurface(0, videomodes.hRes*2, videomodes.vRes*2, 16, masks[1][0], masks[1][1], masks[1][2], masks[1][3]);
		// <- end.
		if (!bscreen)
			bscreen = SDL_CreateRGBSurface(0, videomodes.hRes, videomodes.vRes, 8 * bytes_per_pixel, masks[bytes_per_pixel - 1][0], masks[bytes_per_pixel - 1][1], masks[bytes_per_pixel - 1][2], masks[bytes_per_pixel - 1][3]); // 24bit mask
